...
class Agent {
  ...
  factory Agent.fromDoc(DocumentSnapshot doc) {
    ...
    // try {
    //   agent.cnpj = doc['cnpj'];
    //   agent.openingHours = doc['opening_hours'];
    //   agent.description = doc['description'];
    //   agent.category = doc['category'];
    // } catch (e) {
    //   FirebaseFirestore.instance.collection('agents').doc(doc.id).update({
  ...
  Map<String, dynamic> toJson(Agent? model) => model == null
      ...
      : {
          ...
          "online": model.online,
          "account": account,
          "account_digit": model.accountDigit,
          "agency_digit": model.agencyDigit,
          "operation": model.operation,
        };
}
